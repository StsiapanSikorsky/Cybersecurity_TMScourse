...
## 2) SSH Best Practice
### 2.1 Генерация и добавление ключей  
Для генерации ключа на ВМ KaliLinux используется следующая команда:  
...
_2_. Отключение аутентификации в HostKey  
_3_. Отключение LogLevel (для понимания кто и куда заходил)  
_4_. Ставим количество попыток входа, и коичество сессий  
_5_. Включение PermitRootLogin (никто не подключится под root)  
_6_. Включение PubkeyAuthentication (вход по публичному ключу)  
